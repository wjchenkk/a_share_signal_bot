...
本项目用于生成 A 股尾盘/收盘买入信号、ETF 独立策略信号、ETF 轮动组合、基金定投计划，以及买入后的持仓/交易生命周期建议。所有脚本只输出信号、计划和风控建议，不自动下单。

主要入口：

- `main.py`：A 股股票池尾盘/收盘买入信号。
- `backtest.py`：个股策略历史回测。
- `etf_pool.py`、`etf_strategy.py`、`etf_rotation.py`：ETF 建池、买点信号和轮动组合。
- `fund_dca.py`：基金定投筛选与金额计划。
- `portfolio_manager.py`、`position_monitor.py`、`trade_manager.py`：持仓维护、盘中监控和买入后生命周期管理。
- `stockbot_chat.sh`：小龙虾/对话入口。
...
ETF 建池会抓取全市场 ETF 实时列表，过滤货币/现金类工具和低流动性标的，再按宽基、行业、
跨境、防守、商品等资产类别做配额，避免池子被单一主题占满。输出：

```text
etf_pool.csv                                  生成给ETF策略读取的本地池文件
etf_output/latest_etf_pool_report.md          ETF池构建报告
etf_output/latest_etf_pool_candidates.csv     全部候选及过滤原因
etf_output/latest_etf_pool_selected.csv       入池ETF明细
cache/etf_pool/latest_etf_spot_SOURCE.csv     ETF列表缓存
```
...
盘中持仓监控使用 `position_monitor.py`，偏实时风控预警：
...
买入后生命周期使用 `trade_manager.py`，和严格回测保持一致：T 日生成信号，T+1 开盘买入，T+1 买入当天不提示卖出，T+2 起管理持仓；盘中触发止损、1.5R 半仓止盈、3R 清仓；收盘确认触发 MA20/MA60 破位、大盘弱势或最大持仓天数退出；满足盈利、多头排列且不过热时才给小步加仓建议。
...
`trade_manager.py` 是正式交易计划的主链路，`position_monitor.py` 更适合作为辅助盘中预警；两者都只输出建议，不自动下单。
