...
# ETF 实盘持仓/调仓计划：先生成当期买点和轮动目标，再按回测同频规则输出真实持仓差额。
if printf '%s' "$msg" | grep -Eq 'ETF.*(持仓|交易计划|调仓|再平衡|买卖|后续)|etf.*(持仓|交易计划|调仓|rebalance|买卖|后续)'; then
  ensure_etf_pool
  ETF_PORTFOLIO="${ETF_PORTFOLIO:-etf_portfolio.csv}"
  if ! ACCOUNT="$ACCOUNT" ETF_POOL="$ETF_POOL" ETF_OUT=etf_output CONFIG=config.example.yml ETF_PORTFOLIO="$ETF_PORTFOLIO" ./run_etf_daily.sh \
    > etf_output/last_etf_trade_chat_run.log 2>&1; then
    echo "ETF持仓/调仓计划执行失败："
    tail -120 etf_output/last_etf_trade_chat_run.log || true
    exit 1
  fi
  cat etf_output/last_etf_trade_chat_run.log
  exit 0
fi
